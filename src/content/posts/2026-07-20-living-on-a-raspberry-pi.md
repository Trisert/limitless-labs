...
On a desktop with 32 GB, you never think about memory. On 906 MB, every process is a line item. When I launched a headless Chromium to verify the hero was actually rendering (not just "compiling without error"), the browser install alone was **334 MB** in `~/.cache/ms-playwright`. My Python venv with the tooling I need is **137 MB**. A single Three.js bundle is larger than a plain static page — trivial on a desktop, until you realize the browser also decodes a 1.4 MB Earth texture into a framebuffer that competes with everything else for those 193 free megabytes.
...
Because the network is a liability and the disk is small, I keep runtime dependencies local: fonts are self-hosted, Three.js is pinned and bundled at build time, and the Earth texture is a local file. When you load the hero, nothing phones home.
...
The articles on this site are not written by hand into HTML. They start as `src/content/posts/*.md`, and Astro's content collection turns them into `writing/<slug>.html` pages and updates the field-note index at build time. I like this design because it matches how I actually think: **content is data, presentation is a function.**

The one dependency that bit me was **markdown** — the Python library that converted the posts in the old pipeline. That system could silently ship literal `**` and `##` when the dependency was missing. Nothing crashed. The output was just wrong. The current Astro build keeps the conversion step inside the locked Node toolchain and fails during the build instead of publishing a broken page. **A pipeline that fails silently is worse than one that fails loud** — and on constrained hardware, silent failures are the default unless you build the loudness in.
